...
def hierro(parent):
    ...
    ttk.Label(box1, text="Tabla con piezas De Hierro ", font=("Arial", 15, "bold", "underline"), ).grid(row=0, column=0, sticky="w", pady=15)
    ...
    ttk.Button(box1, text="Limpiar Tabla", command=lambda: limpiar_tabla(mostrar_piezas),bootstyle="secondary-outline").grid(row=2, columnspan=2)

    box2 = ttk.Frame(frame, width=300, )
    box2.grid(row=0, column=1, padx=30)
    
    ttk.Label(box2, text="Fundición de Hierro", font=("Arial", 30, "bold"),).grid(row=0, column=0)
    ttk.Label(box2, text="Mostrar todas las piezas de hierro y seleccionar una pieza de la tabla", font=("Arial", 12, "bold"), wraplength=300, ).grid(row=1, column=0)

    ttk.Button(box2, padding=12, text="Mostrar Piezas", bootstyle="info",command=lambda: mostrar_categoria(mostrar_piezas, "Hierro", "piezas_brutas", detalles_piezas, pieza_seleccionada, imagen_piezas)).grid(row=2, column=0)
    ...
    btn_agregar = ttk.Button(box_btn, width=10, text="AGREGAR", bootstyle="success", command=lambda: agregar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "Hierro", "piezas_brutas"))
    btn_agregar.grid(row=0, column=1, pady=10, padx=5)

    btn_eliminar = ttk.Button(box_btn, width=10, text="ELIMINAR", bootstyle="danger" , command=lambda: eliminar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "Hierro", "piezas_brutas"), padding=10)
