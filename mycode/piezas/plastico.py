...
def plastico(parent):
    ...
    stylo_ventana.configure('Pestania.TFrame', background='#192965')
    ...
    ttk.Label(box1, text="Tabla de piezas", font=("Arial", 15, "bold"), background="#192965", foreground='white').grid(row=0, column=0, sticky="w", pady=15)
    ...
    tk.Label(box2, text="Plastico - inyector", font=("Arial", 30, "bold"), background="#192965", foreground='white').grid(row=0, column=0)
    tk.Label(box2, fg="#535c68", text="Mostrar todas las piezas de plástico y seleccionar una pieza de la tabla", font=("Arial", 12, "bold"), wraplength=300, background="#192965", foreground='white').grid(row=1, column=0)

    tk.Button(box2, text="Mostrar Piezas", padx=20, pady=5, font=("Arial", 14, "bold"), bg="blue", fg="white",  command= lambda: mostrar_categoria(mostrar_piezas, "Plastico", "piezas_brutas", detalles_piezas, pieza_seleccionada, imagen_piezas)).grid(row=2, column=0)
    ...
    btn_agregar = tk.Button(box_btn, width=10, text="Agregar", font=("Arial", 14, "bold"), bg="green", fg="white", command=lambda: agregar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "Plastico", "piezas_brutas"))
    btn_agregar.grid(row=0, column=1, pady=10, padx=5)

    btn_eliminar = tk.Button(box_btn, width=10, text="Eliminar", font=("Arial", 14, "bold"), bg="red", fg="white", command=lambda: eliminar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "Plastico", "piezas_brutas"))
    ...
    detalles = tk.Frame(box_acciones, highlightthickness=1, highlightbackground="#535c68", height=40, background="#192965")
    detalles.grid(row=5, columnspan=2, pady=10, sticky="nw")

    tk.Label(detalles, text="Detalles de las Piezas:", font=("Arial", 9, "bold", 'underline'), background="#192965", foreground='white').grid(row=0, column=0, sticky="nw")

    box_info = tk.Frame(detalles, background="#192965")
    ...
    pieza_seleccionada.img_ruta = "img_piezas/default.png"
