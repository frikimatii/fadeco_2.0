...
def shop(parent):
    ...
    ttk.Label(box1, text="Tabla con piezas Compradas", font=("Arial", 15, "bold", "underline"), ).grid(row=0, column=0, sticky="w", pady=15)
    ...
    ttk.Label(box2, text="Compras", font=("Arial", 30, "bold"), ).grid(row=0, column=0)
    ttk.Label(box2, text="Mostrar todas las piezas de que se compran y seleccionar una pieza de la tabla", font=("Arial", 12, "bold"), wraplength=350, ).grid(row=1, column=0)

    ttk.Button(box2, padding=12, text="Mostrar Piezas", bootstyle="info", command=lambda: mostrar_categoria(mostrar_piezas, "shop", "piezas_brutas", detalles_piezas, pieza_seleccionada, imagen_piezas)).grid(row=2, column=0 ,padx=20, pady=5)
    ...
    box_btn = ttk.Frame(box_acciones,)
    box_btn.grid(row=3, columnspan=2)

    btn_agregar = ttk.Button(box_btn, width=10, text="AGREGAR", bootstyle="success", command=lambda: agregar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "shop", "piezas_brutas"), padding=10)
    btn_agregar.grid(row=0, column=1, pady=10 ,padx=5)
    
    btn_eliminar = ttk.Button(box_btn, width=10, text="ELIMINAR", bootstyle="danger", command=lambda: eliminar_piezas(pieza_seleccionada, entry_cantidad_piezas, mostrar_piezas, historial, "shop", "piezas_brutas"), padding=10)
