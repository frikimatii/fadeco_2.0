...
SELECT PIEZAS, CANTIDAD FROM piezas_brutas WHERE ORIGEN = 'corte' AND PROSESO = 'soldado' UNION SELECT PIEZAS, CANTIDAD FROM piezas_terminadas WHERE PROVEDOR = 'soldador';
"""
...
def ventana_soldador(parent):
    ...
    tk.Button(stock_frame, text="Stock en fábrica", command=lambda: mostrar_categoria(mostrar_piezas, info_pieza, piezas , imagen_piezas, query_mostrar_piezas_soldador)).grid(row=1, column=0, padx=5, pady=5)
    tk.Button(stock_frame, text="Stock en Soldador", command=lambda: mostrar_categoria(mostrar_piezas, info_pieza, piezas, imagen_piezas, query_mostras_bases_ensoldador)).grid(row=1, column=1, padx=5, pady=5)
    ...
    tk.Button(acciones_frame, text="inox 330", command=lambda: mostrar_por_modelo("inox_330", mostrar_piezas)).grid(row=1, column=0, padx=5, pady=5)
    tk.Button(acciones_frame, text="inox 300", command=lambda: mostrar_por_modelo("inox_300", mostrar_piezas)).grid(row=1, column=1, padx=5, pady=5)
    tk.Button(acciones_frame, text="inox 250", command=lambda: mostrar_por_modelo("inox_250", mostrar_piezas)).grid(row=1, column=2, padx=5, pady=5)
    tk.Button(acciones_frame, text="Pintada 330", command=lambda: mostrar_por_modelo("pintada_330", mostrar_piezas)).grid  (row=2, column=0, padx=5, pady=5)
    tk.Button(acciones_frame, text="Pintada 300", command=lambda: mostrar_por_modelo("pintada_330", mostrar_piezas)).grid  (row=2, column=1, padx=5, pady=5)
    tk.Button(acciones_frame, text="ECO", command=lambda: mostrar_por_modelo("ECO", mostrar_piezas)).grid(row=2, column=2, padx=5, pady=5)
    ...
    cabezales_terminados = ttk.Frame(box3, style='Pestania.TFrame', padding=5)
    ...
    btn_agregar_cabezal_inox = ttk.Button(stock_cabezal, text="INOX", style="TButton", command=lambda: mostrar_piezas_tablas(mostrar_piezas, query_cabezales_inox))
    btn_agregar_cabezal_inox.grid(row=1, column=0, padx=5, pady=5)
    
    btn_agregar_cabezal_pintada = ttk.Button(stock_cabezal, text="PINTADA", style="TButton", command=lambda: mostrar_piezas_tablas(mostrar_piezas, query_cabezales_pintada))
    btn_agregar_cabezal_pintada.grid(row=1, column=1, padx=5, pady=5)
    
    btn_agregar_cabezal_250 = ttk.Button(stock_cabezal, text="250", style="TButton", command=lambda: mostrar_piezas_tablas(mostrar_piezas, query_cabezales_250))
