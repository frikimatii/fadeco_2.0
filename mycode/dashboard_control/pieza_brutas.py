...
consultas = {
    "todas": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas",
    "aluminio": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS IN ('Aluminio', 'Hierro_fundicion')",
    "acero_dulce": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS IN ('Acero_dulce', 'Tubo')",
    "inox": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS = 'Chapa_Inox'",
    "chapa_negra": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS = 'Chapa_negra' ",
    "varios": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS IN ('Plastico', 'Tornilleria')",
    "bases": "SELECT PIEZAS, CANTIDAD, STOCK_DESEADO, DETALLES FROM piezas_brutas WHERE CARACTERISTICAS IN ('Caja', 'Pieza_base')"
...
def seleccionar_pieza(event):
    ...
    if seleccion:
        ...
        if values:
            ...
            if conn:
                cursor = conn.cursor()
                cursor.execute("SELECT DETALLES FROM piezas_brutas WHERE PIEZAS = ?", (values[0],))
...
def actualizar_stock():
    ...
    if seleccion:
        ...
        try:
            conn = conectar_db()
            cursor = conn.cursor()
            cursor.execute("UPDATE piezas_brutas SET STOCK_DESEADO = ? WHERE PIEZAS = ?", (nuevo_stock, pieza))
            conn.commit()
            
            # Mostrar solo la pieza actualizada
            cursor.execute("SELECT PIEZAS, CANTIDAD, STOCK_DESEADO FROM piezas_brutas WHERE PIEZAS = ?", (pieza,))
...
# Ventana principal
def ventana_piezas_brutas(parent):
    ...
    ttk.Button(filtro_frame, text="Todas", command= lambda: mostrar_datos(treeview, consultas["todas"])).grid(row=0, column=0, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Aluminio/Hierro", command= lambda: mostrar_datos(treeview, consultas["aluminio"]) ).grid(row=0, column=1, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Acero Dulce/Tubo", command= lambda: mostrar_datos(treeview, consultas["acero_dulce"])).grid(row=1, column=1, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Inox", command= lambda: mostrar_datos(treeview, consultas["inox"])).grid(row=0, column=3, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Chapa_negra", command= lambda: mostrar_datos(treeview, consultas["chapa_negra"])).grid(row=1, column=0, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Varios", command= lambda: mostrar_datos(treeview, consultas["varios"])).grid(row=0, column=2, padx=5, pady=5)
    ttk.Button(filtro_frame, text="Bases", command= lambda: mostrar_datos(treeview, consultas["bases"])).grid(row=1, column=3, padx=5, pady=5)
